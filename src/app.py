...
def repair_tag_text(value: object) -> str:
    """尝试恢复 UTF-8 被错误按 GBK/GB18030 解码后的中文。"""
    ...
    if not any(char in _MOJIBAKE_CHARS for char in text):
        return text
...
TEMPLATES_PATH = DEFAULT_TRAIN_TEMPLATES_PATH
...
class WelcomePage(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.btn_new.setToolTip("创建新的 AutoLabel 项目")
        set_button_role(self.btn_new, "primary")
        intro_layout.addWidget(self.btn_new)

        self.btn_open = QPushButton(icon("open_project"), "打开项目")
        self.btn_open.setMinimumHeight(38)
        self.btn_open.setToolTip("打开本地已有项目")
        set_button_role(self.btn_open, "secondary")
        intro_layout.addWidget(self.btn_open)

        intro_layout.addStretch(1)

        self._recent_summary = QLabel()
        self._recent_summary.setStyleSheet(text_style("hint"))
        self._recent_summary.setWordWrap(True)
        intro_layout.addWidget(self._recent_summary)

        layout.addWidget(sidebar, 0)

        recent_panel = QWidget()
        recent_panel.setObjectName("welcomeRecentPanel")
        ...
        recent_label.setStyleSheet(text_style("title"))
        recent_header.addWidget(recent_label)
        recent_header.addStretch()

        self._recent_count_label = QLabel()
        ...
        self._search_edit.setPlaceholderText("搜索项目名称、路径或任务类型")
        self._search_edit.textChanged.connect(lambda _text: self.refresh_recent_projects())
        recent_layout.addWidget(self._search_edit)

        self.recent_list = QListWidget()
        self.recent_list.setMinimumWidth(520)
        ...
        self.recent_list.setUniformItemSizes(False)
        self.recent_list.setContextMenuPolicy(Qt.CustomContextMenu)
        self.recent_list.customContextMenuRequested.connect(self._on_recent_context_menu)
        self.recent_list.itemSelectionChanged.connect(self._update_recent_summary)
        ...
        self._empty_label.setStyleSheet(
            ...
            f" border: 1px dashed {PALETTE['line']};"
            " border-radius: 8px;"
            " padding: 24px;"
        )
    ...
    def refresh_recent_projects(self) -> None:
        ...
        for project_path in self._config.recent_projects:
            info = self._read_recent_project_info(project_path)
            haystack = " ".join(
                [info["name"], info["path"], info["task_type"]]
            ).lower()
            if query and query not in haystack:
                continue

            item = QListWidgetItem(project_path)
            row = self._make_recent_project_row(info)
        ...
        if self.recent_list.currentItem() is None and visible_count:
        ...
        if hasattr(self, "_recent_count_label"):
            if query:
                ...
                self._recent_count_label.setText(f"{total_count} 个项目")
    ...
    def _make_recent_project_row(self, info: dict[str, str]) -> QWidget:
        row = QWidget()
        row.setObjectName("recentProjectRow")
        row.setStyleSheet(
            "QWidget#recentProjectRow {"
            f" background-color: {PALETTE['panel_alt']};"
            f" border: 1px solid {PALETTE['line']};"
            " border-radius: 8px;"
            "}"
            "QWidget#recentProjectRow QWidget {"
            " background-color: transparent;"
            "}"
            "QWidget#recentProjectRow QLabel {"
            " background-color: transparent;"
            " border: none;"
            "}"
            "QWidget#recentProjectRow:hover {"
            f" background-color: {PALETTE['panel_raised']};"
            f" border-color: {PALETTE['line_strong']};"
            "}"
        )
        ...
        path_label.setStyleSheet(
            f"color: {PALETTE['text_subtle']}; font-size: 12px;"
        )
        ...
        task_label = QLabel(info["task_type"])
        task_label.setAlignment(Qt.AlignCenter)
        task_label.setMinimumWidth(72)
        ...
        layout.addWidget(task_label, 0, Qt.AlignVCenter)

        return row

    def _update_recent_summary(self) -> None:
        if not hasattr(self, "_recent_summary"):
            return
        item = self.recent_list.currentItem()
        if item is None:
            count = len(self._config.recent_projects)
            self._recent_summary.setText(f"最近项目: {count}")
            return
        info = self._read_recent_project_info(item.text())
        self._recent_summary.setText(
            f"{info['name']}\n{info['task_type']} · {info['status']}"
        )
    ...
    def _on_recent_context_menu(self, pos) -> None:
        ...
        open_dir_action = menu.addAction("打开资源目录")
        remove_action = menu.addAction("移除项目")
        action = menu.exec_(self.recent_list.viewport().mapToGlobal(pos))

        if action == edit_action:
            self.edit_project_requested.emit(project_path)
        elif action == open_dir_action:
            self._open_project_directory(project_path)
        elif action == remove_action:
            self.remove_project_requested.emit(project_path)

    def _open_project_directory(self, project_path: str) -> None:
        path = Path(project_path)
        if not path.exists():
            QMessageBox.warning(self, "打开失败", f"项目目录不存在:\n{project_path}")
            return
        try:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, config_path: Path | str | None = None, parent=None):
        ...
        geo = self._startup_geometry(self._app_config.window_geometry)
        ...
        self._tag_ctrl.tags_changed.connect(self._refresh_train_filter_summary)
        self._tag_ctrl.image_tags_changed.connect(self._refresh_train_filter_summary)
        ...
        self._tools_panel: ScriptToolPanel | None = None
    ...
    def _startup_geometry(saved: dict[str, int]) -> dict[str, int]:
        """Return a comfortable non-maximized startup geometry."""
        default = dict(DEFAULT_WINDOW_GEOMETRY)
        try:
            geo = {
                "x": int(saved.get("x", default["x"])),
                "y": int(saved.get("y", default["y"])),
                "width": int(saved.get("width", default["width"])),
                "height": int(saved.get("height", default["height"])),
            }
        ...
        max_w = int(available.width() * 0.86)
        max_h = int(available.height() * 0.86)
        too_large = geo["width"] > max_w or geo["height"] > max_h
        offscreen = (
            geo["x"] < available.left()
            or geo["y"] < available.top()
            or geo["x"] + geo["width"] > available.right() + 1
            or geo["y"] + geo["height"] > available.bottom() + 1
        )
        if too_large or offscreen:
            geo["width"] = min(default["width"], max_w)
            geo["height"] = min(default["height"], max_h)
            geo["x"] = available.left() + max(0, (available.width() - geo["width"]) // 2)
    ...
    def _on_tab_changed(self, index: int) -> None:
        ...
        elif self._preview_panel is not None and current is self._preview_panel:
    ...
    def _sync_train_config_from_disk(self) -> None:
        ...
        if self._train_panel is None:
            return
        disk_config = AppConfig.load(self._config_path)
    ...
    def _setup_menus(self) -> None:
        ...
        self._tools_menu = mb.addMenu("工具")

        self._structure_action = QAction("模型结构查看器", self)
        self._structure_action.triggered.connect(self._on_open_structure_viewer)
        self._tools_menu.addAction(self._structure_action)
    ...
    def _maybe_apply_detected_project_data(self, project_manager: ProjectManager) -> None:
        ...
        current = current_project_data_candidate(project_manager)
        same_config = (
            detected.image_dir == project_manager.config.image_dir
            and detected.label_dir == project_manager.config.label_dir
        )
        if same_config:
            return
        if current is not None and current.score >= detected.score:
            return

        msg = (
            "检测到项目目录里有图片和同名 JSON 标注。\n\n"
            f"图片目录: {detected.image_dir}\n"
            f"标注目录: {detected.label_dir}\n"
            f"图片: {detected.image_count} 张\n"
            f"匹配 JSON: {detected.matched_json_count} 个\n\n"
            "是否切换项目配置并加载这些数据？"
        )
    ...
    def _on_edit_recent_project(self, project_path: str) -> None:
        try:
            pm = ProjectManager.open(project_path)
        except (FileNotFoundError, json.JSONDecodeError, KeyError, OSError) as e:
            QMessageBox.warning(self, "修改失败", f"无法打开项目: {e}")
            return

        dlg = QDialog(self)
        dlg.setWindowTitle("修改项目")
        form = QFormLayout(dlg)

        name_edit = QLineEdit(pm.config.name)
        ...
        if not new_name:
            QMessageBox.warning(self, "名称无效", "项目名称不能为空。")
            return

        pm.config.name = new_name
        pm.config.task_type = task_combo.currentText()
        try:
            pm.save()
        except OSError as e:
        ...
        if self._project is not None and self._project.project_dir == pm.project_dir:
            self.open_project(pm)
        self._welcome.refresh_recent_projects()
        self._status_label.setText(f"已修改项目: {pm.config.name}")

    def _on_remove_recent_project(self, project_path: str) -> None:
        reply = QMessageBox.question(
            self,
            "移除项目",
            "将从最近项目中移除。项目文件、图片、标注和模型文件不会删除。是否继续？",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if reply != QMessageBox.Yes:
        ...
        self._app_config.recent_projects = [
            p for p in self._app_config.recent_projects if p != project_path
        ]
    ...
    def _on_export(self) -> None:
        ...
        if self._label_panel:
            self._label_panel.save_and_cleanup()
        try:
            export_dir = self._project_ctrl.export(self._project)
    ...
    def _sync_available_tags(self) -> None:
        ...
        raw_tags = [
            str(tag).strip()
            for tag in self._project.config.tags
            if str(tag).strip()
        ]
        ...
        for raw_tag in raw_tags:
            fixed_tag = repair_tag_text(raw_tag)


            if fixed_tag and fixed_tag not in fixed_tags:
                fixed_tags.append(fixed_tag)

        logger.warning("原始项目 Tags：%r", raw_tags)
        logger.warning("修复后的 Tags：%r", fixed_tags)



        if fixed_tags != raw_tags:
            self._project.config.tags = fixed_tags

            try:
                self._project.save()
                logger.info("已保存修复后的项目 Tags")
            except OSError:
                logger.exception("保存修复后的项目 Tags 失败")
        ...
        if self._preview_panel is not None:
            self._preview_panel.set_available_tags(fixed_tags)


        if self._train_panel is not None:
            self._train_panel.set_available_tags(fixed_tags)
            self._refresh_train_filter_summary()

    def _sync_train_available_classes(self) -> None:
        if self._project is None or self._train_panel is None:
            return
    ...
    def _on_label_user_tags_changed(self, path, tags) -> None:
        ...
        if self._preview_panel is not None and self._project is not None:
            self._preview_panel.set_available_tags(list(self._project.config.tags))
            self._preview_panel.refresh()
        self._refresh_train_filter_summary()
    ...
    def _on_model_load(self, model_id: str) -> None:
        ...
        try:
            if self._model_ctrl.load_model(model_id):
                ...
                detected_imgsz = self._model_ctrl.current_model_imgsz()
                if self._model_panel and model_info:
                    self._model_panel.set_current_model_name(model_info.name)
                    if detected_imgsz is not None:
                        self._model_panel.set_predict_imgsz(detected_imgsz)
                if model_info:
                    status = f"已加载模型: {model_info.name}"
                    if detected_imgsz is not None:
                        status = f"{status} | imgsz={detected_imgsz}"
                    self._status_label.setText(status)
                else:
                    self._status_label.setText("")
        finally:
            if self._model_panel:
                self._model_panel.set_busy(False)
    ...
    def _on_model_export_onnx(self, model_id: str) -> None:
        if self._model_panel:
            self._model_panel.set_busy(True, "正在导出 ONNX…")
            QApplication.processEvents()
        try:
            path = self._model_ctrl.export_model_onnx_file(model_id)
            if path:
                self._status_label.setText(f"ONNX 导出完成: {path}")
        finally:
            if self._model_panel:
                self._model_panel.set_busy(False)

    def _on_model_export_pt(self, model_id: str) -> None:
        if self._model_panel:
            self._model_panel.set_busy(True, "正在导出 PT…")
            QApplication.processEvents()
        try:
            path = self._model_ctrl.export_model_pt(model_id)
            if path:
                self._status_label.setText(f"PT 导出完成: {path}")
        finally:
            if self._model_panel:
                self._model_panel.set_busy(False)
    ...
    def _on_model_predict_image(self, image_path: str) -> None:
        if not self._project or not self._model_panel:
            return
        if self._model_ctrl.predictor is None:
            QMessageBox.information(self, "提示", "请先在模型面板中加载一个模型")
            return

        source_path = Path(image_path)
        imgsz = self._model_panel.get_predict_imgsz()
        ...
        try:
            if source_path.is_dir():
                ...
                if not image_paths:
                    ...
                    return
                output_parent = self._project.project_dir / "model_predictions"
                saved = self._model_ctrl.predict_native_save(
                    image_paths,
                    output_parent,
                    conf=conf,
                    iou=iou,
                ...
                if saved is None:
                    ...
                    return
                result_dir, result_count = saved
                summary = (
                    f"目录推理完成: {result_count}/{len(image_paths)} 张，"
                    f"结果已保存到 {result_dir}"
                )
                self._model_panel.set_prediction_result_dir(result_dir, summary)
                self._status_label.setText(summary)
                return

            img_path = source_path
            native_result = self._model_ctrl.predict_native_plot(
                img_path,
                conf=conf,
                iou=iou,
                imgsz=imgsz,
            ...
            if native_result is None:
                self._status_label.setText("模型推理: 未生成结果")
                return
            image, summary = native_result
            self._model_panel.set_prediction_native_image(img_path, image, summary)
            self._status_label.setText(f"模型推理: {summary}")
        finally:
            self._model_panel.set_busy(False)
    ...
    def _on_auto_label_single(self) -> None:
        ...
        if self._project.config.task_type == "classify":
            ...
            params = inspect.signature(
                self._model_ctrl.predict_single_classify
            ).parameters
            if "imgsz" in params:
                classify_kwargs["imgsz"] = imgsz
            if "device" in params:
    ...
    def _apply_single_auto_label_result(
        self,
        annotations,
        overlap_iou: float,
    ) -> None:
    ...
    def _refresh_train_filter_summary(self, *_args) -> None:
        """Recompute and push the training data filter summary to TrainPanel.
        ...
        if self._project is None:
            self._train_panel.set_filter_summary(None)
            return
        ...
        try:
            ...
            selected_count = 0
        self._train_panel.set_filter_summary(selected_count, tag_counts)
    ...
    def closeEvent(self, event) -> None:
        if self._tools_panel is not None and not self._tools_panel.prepare_close():
            event.ignore()
            return
        ...
        if self._train_panel is not None:
            self._app_config.last_train_config = self._train_panel.get_last_train_settings()
        if self._model_panel is not None:
            self._app_config.model_panel_config = self._model_panel.get_panel_settings()
            self._app_config.default_conf_threshold = self._model_panel.get_conf_threshold()
            self._app_config.default_iou_threshold = self._model_panel.get_iou_threshold()
            self._app_config.overlap_iou_threshold = self._model_panel.get_overlap_iou_threshold()
        self._app_config.save(self._config_path)
