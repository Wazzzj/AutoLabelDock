...
Three independent widgets; none depends on the others. They are designed to be
...
from src.ui.theme import PALETTE, chip_style, text_style


_CHIP_STYLE = chip_style()
_FILTER_BUTTON_MIN_WIDTH = 112
_FILTER_CONTROL_MIN_HEIGHT = 30
_FILTER_BUTTON_STYLE = f"""
QToolButton {{
...
class TagChipBar(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.setObjectName("tagApplyBar")
    ...
    def _rebuild(self) -> None:
        ...
        for t in self._current:
            ...
            chip.setText(f"{t}  脳" if not self._read_only else t)
...
class TagFilterBar(QWidget):
    ...
    excluded (cycling enforces includes/excludes exclusivity). Conflicts
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self._btn.setMinimumWidth(_FILTER_BUTTON_MIN_WIDTH)
        self._btn.setMinimumHeight(_FILTER_CONTROL_MIN_HEIGHT)
        self._btn.setStyleSheet(_FILTER_BUTTON_STYLE)
...
class TagApplyBar(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self._hint = QLabel("点击 Tag 后按 T 应用到选中图片")
        self._hint.setStyleSheet(text_style("hint"))
        self._layout.addWidget(self._hint)

        self._empty_label = QLabel("无项目 Tag，请在「编辑 > Tag 管理」中添加")
        self._empty_label.setStyleSheet(text_style("hint"))
        self._empty_label.hide()
        self._layout.addWidget(self._empty_label)

        self._apply_btn = QPushButton("批量添加")
        self._apply_btn.setToolTip("将当前选中的 Tag 添加到左侧复选框勾选的图片")
    ...
    def _rebuild(self) -> None:
        ...
        # Insert chips between hint/empty label and the apply button.
        insert_at = max(0, self._layout.indexOf(self._apply_btn))
    ...
    def _restyle_all(self) -> None:
        self._apply_btn.setEnabled(self._armed is not None)
