...
class LabelPanel(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        rs_layout.setContentsMargins(6, 0, 6, 0)
        rs_layout.setSpacing(4)
        refresh_btn = QToolButton()
        refresh_btn.setIcon(icon("refresh"))
        refresh_btn.setToolTip("刷新图像列表 (F5)")
        refresh_btn.setEnabled(False)
        refresh_btn.clicked.connect(self._on_refresh_clicked)
        rs_layout.addWidget(refresh_btn)
    ...
    def _on_view_classes_changed(self) -> None:
        ...
        filter combo and the LocateAnything target-class dropdown."""
        self._refresh_class_filter()
    ...
    def _refresh_class_filter(self) -> None:
        ...
        Preserves the current selection if still present. The option list is
        derived from project classes plus classes found in saved annotations.
        """
        ...
        self._class_filter_combo.addItem("所有类别")
        for cls in merged_project_annotation_classes(self._project):
    ...
    def focus_image(self, path: Path) -> bool:
        if self._view is None:
            return False
        focus = getattr(self._view, "focus_image", None)
        if callable(focus):
            return bool(focus(path))
        return False
    ...
    def _apply_armed_tag(self) -> None:
        ...
        if not tag:
            self.status_changed.emit("先点击上方 Tag，再按 T 应用到选中图片")
            return
        paths = self._view.get_selected_image_paths()
        if not paths:
            self.status_changed.emit("请先在左侧图片列表中选择图片")
            return
        self._view.commit_pending_save()
        n = self._tag_ctrl.apply_tag_to_images(tag, paths)
        self.status_changed.emit(f"已为 {n}/{len(paths)} 张图片添加 Tag: {tag}")

    def _on_apply_tag_shortcut(self) -> None:
    ...
    def _on_images_dropped(self, paths: list[Path]) -> None:
        if not self._project:
            return
        image_dir = self._project.image_root()
        active_folder = self._project.config.active_data_folder
        if active_folder:
            image_dir = image_dir / Path(active_folder)
