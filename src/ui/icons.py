...
from src.core.resources import APP_LOGO_CANDIDATES
...
_SVGS: dict[str, str] = {
    ...
    "polygon": (
        '<svg viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2"'
        ' stroke-linecap="round" stroke-linejoin="round">'
        '<polygon points="5 4 19 6 16 20 7 17 3 10"/>'
...
def app_icon() -> QIcon:
    """Return the application logo icon from configured static resources."""
    ...
    for candidate in APP_LOGO_CANDIDATES:
        if candidate.exists():
            _app_icon_cache = QIcon(str(candidate))
            return _app_icon_cache
