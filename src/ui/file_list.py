...
from PyQt5.QtGui import QColor, QDragEnterEvent, QDropEvent, QKeyEvent, QMouseEvent
...
class FileListWidget(QListWidget):
    ...
    move_images_requested = pyqtSignal(list)     # list[Path]
    ...
    def __init__(self, parent=None):
        ...
        self._display_names: dict[str, str] = {}  # path_str -> text shown in rows
        self._checked_paths: set[str] = set()  # checkbox-based batch selection
        ...
        self.setObjectName("imageFileList")
        self.setStyleSheet(
            "QListWidget#imageFileList::item:hover {"
            " background-color: transparent;"
            "}"
            "QListWidget#imageFileList::item:selected:hover {"
            f" background-color: {PALETTE['primary_soft']};"
            f" color: {PALETTE['text']};"
            "}"
            "QListWidget#imageFileList::indicator:hover {"
            " background-color: transparent;"
            "}"
        )
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if (
            ...
            if anchor is None or anchor < 0 or anchor >= self.count() or self.item(anchor).isHidden():
                anchor = self.currentRow()
            if anchor < 0 or anchor >= self.count() or self.item(anchor).isHidden():
                anchor = clicked_row

            self._select_range(anchor, clicked_row)
    ...
    def _select_range(self, anchor: int, clicked_row: int) -> None:
        """Select and check the visible rows between two row indexes."""
        start, end = sorted((anchor, clicked_row))
        ...
        try:
            for row in range(self.count()):
                ...
                if row_item is None:
                    continue
                in_range = start <= row <= end and not row_item.isHidden()
                row_item.setSelected(in_range)
                row_item.setCheckState(Qt.Checked if in_range else Qt.Unchecked)
                path_str = row_item.data(Qt.UserRole)
                if in_range and path_str:
    ...
    def set_image_paths(self, paths: list[Path]) -> None:
        ...
        for path in paths:
            ...
            if name_counts.get(path.name, 0) > 1 and path.parent.name:
                display_name = f"{path.parent.name}/{path.name}"
            self._display_names[str(path)] = display_name
            item = QListWidgetItem(display_name)
            item.setData(Qt.UserRole, str(path))
            item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
            item.setCheckState(
                Qt.Checked if str(path) in self._checked_paths else Qt.Unchecked
            )
    ...
    def set_status(self, path: Path, status: str) -> None:
        ...
        for i in range(self.count()):
            ...
            if item.data(Qt.UserRole) == str(path):
                display_name = self._display_names.get(str(path), path.name)
    ...
    def set_image_classes(self, path: Path, classes: set[str]) -> None:
        ...
        self._image_classes[str(path)] = set(classes)
        self._refresh_row_visibility(path)
    ...
    def _refresh_row_visibility(self, path: Path) -> None:
        ...
        for i in range(self.count()):
            ...
            if item.data(Qt.UserRole) == path_str:
                self._set_item_hidden_by_filter(item, self._compute_hidden_for_path(path_str))
                return
    ...
    def check_visible_paths(self, checked: bool = True) -> None:
        """Check or uncheck all currently visible image rows."""
    ...
    def _set_item_hidden_by_filter(self, item: QListWidgetItem, hidden: bool) -> None:
    ...
    def get_selected_paths(self) -> list[Path]:
        """Get all checked image paths, falling back to highlighted selection."""
        paths = []
        for i in range(self.count()):
            item = self.item(i)
            if item.checkState() == Qt.Checked:
    ...
    def contextMenuEvent(self, event) -> None:
        ...
        clicked_path = Path(path_str)
        selected = self.get_selected_paths()
        if clicked_path not in selected:
            selected = [clicked_path]
        menu = QMenu(self)

        select_all = menu.addAction("全选可见图片")
        select_all.triggered.connect(lambda: self.check_visible_paths(True))

        clear_checked = menu.addAction("取消勾选")
        clear_checked.triggered.connect(lambda: self.check_visible_paths(False))
        ...
        if len(selected) > 1:
            ...
            move_imgs = menu.addAction(f"移动到数据版本... ({len(selected)} 张)")
            move_imgs.triggered.connect(
                lambda: self.move_images_requested.emit(selected)
            )
            ...
            move_imgs = menu.addAction("移动到数据版本...")
            move_imgs.triggered.connect(
                lambda: self.move_images_requested.emit([clicked_path])
            )

            delete_imgs = menu.addAction("删除图片")
            delete_imgs.triggered.connect(
                lambda: self.delete_images_requested.emit([clicked_path])
            )
            menu.addSeparator()
