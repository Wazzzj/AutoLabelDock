...
from PyQt5.QtCore import QProcess, QProcessEnvironment, QUrl, pyqtSignal
from PyQt5.QtGui import QDesktopServices, QFontDatabase, QKeySequence, QTextCursor
...
2) 在小工具面板选择数据版本后运行本脚本
3) 结果默认输出到项目目录下 crops/，指定数据版本时输出到 crops/<数据版本>/
"""
from __future__ import annotations
...
PROJECT_DIR = Path(os.environ.get("AUTOLABEL_PROJECT_DIR") or ".")
DATA_VERSION = os.environ.get("AUTOLABEL_DATA_VERSION", "").replace("\\\\", "/").strip("/")
ONLY_CONFIRMED = False
KEEP_CLASS_SUBDIR = True
LIST_LIMIT = 20
IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png", ".bmp", ".tiff", ".tif", ".webp"}
IGNORED_DIRS = {"model_predictions"}
...
def mirrored_image_for_label(label_file: Path, label_root: Path, image_root: Path) -> Path | None:
    try:
        rel = label_file.relative_to(label_root)
    except ValueError:
        return None
    stem_path = image_root / rel.with_suffix("")
...
def resolve_image_for_json(
    ...
    mirrored = mirrored_image_for_label(label_file, label_root, image_root)
    ...
    if declared:
        ...
        candidates.append(PROJECT_DIR / declared_path)
        candidates.append(image_root / declared_path)
        for candidate in candidates:
            if candidate.exists():
                return candidate, ""

        by_name = image_by_name.get(declared_path.name, [])
        if len(by_name) == 1:
            return by_name[0], ""
        if len(by_name) > 1:
            return None, f"图片文件名重复: {declared_path.name}"
...
def main() -> None:
    ...
    if DATA_VERSION:
        output_dir = output_dir / safe_name(DATA_VERSION)

    if not image_scan_root.exists():
        print(f"图片目录不存在: {image_scan_root}")
        return
    if not label_scan_root.exists():
        print(f"标签目录不存在: {label_scan_root}")
        return
    ...
    print(f"图片目录: {image_scan_root}")
    print(f"标签目录: {label_scan_root}")
    ...
    for label_file in label_files:
        ...
        image_path, reason = resolve_image_for_json(
            label_file, label_scan_root, image_scan_root, doc, image_by_name, images_by_stem
        )
    ...
    unmatched_images = [
        str(p.relative_to(image_scan_root)) if p.is_relative_to(image_scan_root) else p.name
        for p in images if p not in matched_image_paths
    ]
        ...
        try:
            with Image.open(image_path) as img:
                ...
                for i, ann in enumerate(annotations):
                    ...
                    class_name = safe_name(str(ann.get("class_name", "unknown")))
                    out_dir = output_dir / class_name if KEEP_CLASS_SUBDIR else output_dir
                    out_dir.mkdir(parents=True, exist_ok=True)
                    out_name = f"{safe_name(image_path.stem)}_{i:03d}.jpg"
...
class ScriptToolPanel(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self._data_version_label = QLabel("数据版本:")
        self._data_version_label.setStyleSheet(text_style("hint"))
        self._data_version_combo = QComboBox()
        self._data_version_combo.setMinimumWidth(150)
        self._data_version_combo.setToolTip("选择脚本运行时使用的数据版本；内置裁剪脚本会按此版本裁剪")
        self._btn_clear = QPushButton("清空输出")
        set_button_role(self._btn_clear, "secondary")
        self._btn_open_dir = QPushButton("打开目录")
        set_button_role(self._btn_open_dir, "secondary")
        ...
        btn_layout.addWidget(self._data_version_label)
        btn_layout.addWidget(self._data_version_combo)
    ...
    def _connect_signals(self) -> None:
        ...
        self._btn_open_dir.clicked.connect(self._on_open_directory_clicked)
    ...
    def _ensure_builtin_tools(self) -> None:
        ...
        if crop_path.exists():
            try:
                should_write = _BUILTIN_CROP_VERSION not in crop_path.read_text(encoding="utf-8")
            except OSError:
                should_write = False
        if should_write:
    ...
    def _on_open_directory_clicked(self) -> None:
        if not self._working_dir.exists():
            QMessageBox.warning(self, "打开失败", f"目录不存在: {self._working_dir}")
            return
        QDesktopServices.openUrl(QUrl.fromLocalFile(str(self._working_dir)))
        self.status_changed.emit(f"已打开目录: {self._working_dir}")
    ...
    def set_working_directory(self, path: Path | str | None) -> None:
        ...
        self._cwd_label.setToolTip(str(self._working_dir))
        self._refresh_data_versions()

    def selected_data_version(self) -> str:
        """Return the selected project data-version folder, or empty for all images."""
        value = self._data_version_combo.currentData()
        return str(value or "")

    def _refresh_data_versions(self) -> None:
        current = self.selected_data_version()
        self._data_version_combo.blockSignals(True)
        self._data_version_combo.clear()
        self._data_version_combo.addItem("全部数据", "")

        project_json = self._working_dir / "project.json"
        active_folder = ""
        folders: set[str] = set()
        if project_json.exists():
            ...
            image_dir = Path(config.get("image_dir", "images"))
            if not image_dir.is_absolute():
                image_dir = self._working_dir / image_dir
            if image_dir.exists():
        ...
        for folder in sorted(folders, key=lambda value: (value.count("/"), value.lower())):
            self._data_version_combo.addItem(folder, folder)

        restore = current or active_folder
        index = self._data_version_combo.findData(restore)
        if index >= 0:
            self._data_version_combo.setCurrentIndex(index)
        self._data_version_combo.setEnabled(project_json.exists())
        self._data_version_combo.blockSignals(False)

    @staticmethod
    def _normalize_data_folder(value) -> str:
        text = str(value or "").replace("\\", "/").strip("/")
        parts = [
            part
            for part in text.split("/")
            if part and part not in {".", ".."} and not part.startswith(".")
        ]
        if not parts or parts[0].casefold() == "model_predictions":
            return ""
        return "/".join(parts)
    ...
    def _on_run_clicked(self) -> None:
        ...
        env.insert("AUTOLABEL_PROJECT_DIR", str(self._working_dir))
        env.insert("AUTOLABEL_DATA_VERSION", self.selected_data_version())
        ...
        self._append_line(f"数据版本: {self.selected_data_version() or '全部数据'}")
    ...
    def _set_running_state(self, running: bool) -> None:
        ...
        self._data_version_combo.setEnabled(not running)
