...
from src.core.config import AppConfig, DEFAULT_APP_CONFIG_PATH
...
_APP_CONFIG_PATH = lambda: DEFAULT_APP_CONFIG_PATH
...
class ClassifyView(TaskView):
    ...
    def __init__(
        ...
        self._config_path = Path(config_path) if config_path is not None else _APP_CONFIG_PATH()
        self._project: ProjectManager | None = None
        self._classes: list[str] = []
    ...
    def _apply_class(self, class_name: str) -> None:
        if not self._project:
            return
        selected_items = self._selected_visible_items()
    ...
    def _selected_or_focused_items(self) -> list:
        items = self._selected_visible_items()
        if items:
            return items
        current = self._grid.currentItem()
        return [current] if current is not None and not current.isHidden() else []
    ...
    def get_selected_image_paths(self) -> list[Path]:
        return [Path(it.data(Qt.UserRole)) for it in self._selected_visible_items()]
    ...
    def _refresh_item_visibility(self, path: Path) -> None:
        ...
        if item is None:
            return
        ia = load_annotation(self._project.label_path_for(path))
        self._set_item_hidden_by_filter(item, self._compute_hidden(ia))

    def _selected_visible_items(self) -> list[QListWidgetItem]:
        return [it for it in self._grid.selectedItems() if not it.isHidden()]
    ...
    def focus_image(self, path: Path) -> bool:
        item = self._grid.item_for_path(Path(path))
        if item is None:
            return False
        self._grid.setCurrentItem(item)
        self._grid.scrollToItem(item, QAbstractItemView.PositionAtCenter)
        return True
