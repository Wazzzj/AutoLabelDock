...
def _move_popup_onscreen(widget: QDialog, anchor_pos) -> None:
    """Place a popup near an anchor point while keeping it inside the screen."""
    widget.adjustSize()
    size = widget.sizeHint().expandedTo(widget.minimumSizeHint())
    desktop = QApplication.desktop()
    screen = desktop.screenNumber(anchor_pos)
    if screen < 0:
        screen = desktop.primaryScreen()
    available = desktop.availableGeometry(screen)

    margin = 8
    x = anchor_pos.x()
    y = anchor_pos.y()
    if x + size.width() > available.right() - margin:
        x = anchor_pos.x() - size.width()
    if y + size.height() > available.bottom() - margin:
        y = anchor_pos.y() - size.height()

    x = max(available.left() + margin, min(x, available.right() - size.width() - margin))
    y = max(available.top() + margin, min(y, available.bottom() - size.height() - margin))
    widget.move(x, y)
