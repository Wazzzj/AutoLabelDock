...
_INVALID_RUN_NAME_CHARS = re.compile(r'[<>:"/\\|?*\x00-\x1f]+')


def _clean_run_name(value: str) -> str:
    """Return a filesystem-safe Ultralytics run name."""
    cleaned = _INVALID_RUN_NAME_CHARS.sub("_", value.strip())
    cleaned = re.sub(r"\s+", " ", cleaned)
    return cleaned.strip(" ._")


def _unique_run_name(models_dir: Path, preferred: str) -> str:
    """Avoid clobbering an existing Ultralytics output directory."""
    if not (models_dir / preferred).exists():
        return preferred
    index = 2
    while (models_dir / f"{preferred}-{index}").exists():
        index += 1
...
class TrainController:
    ...
    def start(
        ...
        requested_name = _clean_run_name(config.name)
        run_name = requested_name or f"{task}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        run_name = _unique_run_name(models_dir, run_name)
    ...
    def register_model_after_training(self, metrics: dict) -> ModelInfo | None:
        ...
        model_info = ModelInfo(
            name=self._run_name or f"{task}-{len(registry.list_models()) + 1}",
            path=model_path,
            task=task,
