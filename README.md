...
- 检测框、OBB 和分割多边形在标注画布及预览大图中显示“类别 + 面积百分比 + 像素面积”；缩略图显示类别和面积百分比。检测框按宽高计算，OBB / 分割按真实多边形面积计算。
- 单图独立撤销 / 重做栈，默认深度 50。
- 切图自动保存，图片使用 LRU 缓存。
- 图片列表按状态区分：未标注、待确认、已确认。
- 支持拖拽导入图片、右键批量确认、批量删除、批量移动到数据文件夹。
- 勾选或选中多张图片后修改类别，会将这些图片中与当前旧类别相同的标注批量替换为新类别。
- 删除当前图片后会就近选中下一张（末尾则选择上一张）并保持列表滚动位置；拖动待确认框时仅实时重绘画布，松开后再同步标注面板。
- 支持按状态、类别和自定义标签组合过滤。
- Catppuccin Mocha 深色主题。

### 数据预览与高级筛选

- 预览页支持按标注状态、类别、数据版本和高级条件组合筛选。
- 高级筛选统一包含 Tag、宽度、高度、面积、置信度、中心点 X / Y 的最小值和最大值，并可一键重置全部条件。
- 宽度、高度和中心位置按图片尺寸百分比计算，面积按图片面积百分比计算，因此不同分辨率的图片可以使用同一套阈值。
- 类别和多个属性条件必须由同一个标注同时满足；未勾选的范围不参与过滤，直接修改范围会自动启用对应条件。
- 分类项目没有目标框，仅保留高级筛选中的图片 Tag 条件。
...
- 模型推理页提供“每张图片仅保留置信度最高的 1 个 ROI 框”可选项，默认关闭；启用后同一规则会同步应用于当前推理、单图自动标注和批量标注。
- 批量推理在后台线程运行，逐图落盘，可取消。
- 新预测会和已有同类已确认标注按 IoU 匹配，避免重复框。
- 自动标注结果默认处于“待确认”状态；人工编辑或确认后转为已确认。
- 调整待确认框后可快速来回切换图片；图片与标注会成组更新，并取消上一张图遗留的拖拽状态，避免旧标注延迟显示或检测框错位。
...
- 一键准备 YOLO 训练数据集。
- 支持 detect、segment、obb、pose、classify 五类 YOLO 任务。
- 训练在独立子进程中运行，CUDA、DataLoader 或第三方原生库异常不会直接关闭标注主界面；模型文件已完整保存时可自动恢复并完成注册。
- 按主类别进行 train / val 分层抽样。
- 数据集图片优先使用符号链接，失败后自动降级为硬链接或复制。
- 支持训练模板和完整超参数配置。
- 数据增强预览同时显示单图增强和完整训练采样，支持几何变换、HSV、翻转、Mosaic、MixUp、Copy-Paste、分类策略与 Erasing，并同步叠加变换后的标注轮廓。
...
- 推理设置中的最高置信度 ROI 规则会持久化保存；批量任务启动后锁定本次选项状态，保证整批图片使用一致规则。
...
| 导出格式 | 输出结构 |
|:---|:---|
| labelme / X-AnyLabeling Detect / X-AnyLabeling OBB / iSAT | 图片与同名 JSON 并排 |
| roLabelImg OBB | 图片与同名 XML 并排 |
| YOLO | `images/` 与 `labels/` 分开，并生成 `data.yaml` |
| COCO | `images/` 与 `annotations/coco.json` 分开 |
| ImageFolder | 直接按类别建立图片目录，不额外生成 `images/` 或 `labels/` |

iSAT 导出会按图片子目录将图片和同名 JSON 放在一起，写入标准 `info`、`objects`、像素级 `segmentation`、`bbox`、`area`、`group` 和 `layer` 字段。多边形会保留真实轮廓；只有矩形框的标注会转换为四点多边形，同时支持“仅导出已确认标注”。
...
OBB 导入和导出明确拆分为两种格式：roLabelImg XML 使用 `robndbox` 的 `cx`、`cy`、`w`、`h`、`angle` 字段，角度按弧度处理；X-AnyLabeling JSON 使用四个像素坐标点、`shape_type: "rotation"` 和 `direction` 弧度角。两种格式均保留图片子目录结构，导出时图片与同名标注文件放在一起。

OBB 标注可按 `O` 或在画布右键菜单选择“旋转框”，按住鼠标左键直接拖出矩形。创建后选中旋转框，拖动框外的圆形旋转手柄可绕中心整体旋转；拖动四角可像普通矩形框一样缩放，并始终保持矩形形状。

新建或重新打开 OBB 项目时，程序会检查项目根目录、图片目录以及常见标注目录中的同名 XML/TXT。若内部 `labels/*.json` 尚不存在，会自动选择匹配未标注图片最多的侧边标注格式并转换；XML 与 TXT 匹配数相同时优先 XML，已有内部标注不会被覆盖。
...
1. 新建项目：选择任务类型 `detect`、`segment`、`obb`、`pose` 或 `classify`，填写项目目录和初始类别。
...
分类标注页采用面向数据整理的工作台布局：类别按钮和类别图片数位于缩略图上方；批量工具栏实时显示总数、可见数、已标注数和已选数，并提供全选可见、反选、取消选择、清除类别和批量删除。按住 `Ctrl` 或 `Shift` 可多选图片，随后点击类别按钮即可批量改类；批量删除始终需要二次确认。
